...
ProjectWebRouter.post('/', async (req, res) => {
...
ProjectWebRouter.put('/:id', async (req, res) => {
    ...
    if (!updatedProjectWeb) return res.status(404).json({ message: "Projet introuvable" });
    res.status(200).json(updatedProjectWeb);
})
ProjectWebRouter.delete('/:id', async (req, res) => {
    try {
        const deletedProjectWeb = await ProjectWeb.findByIdAndDelete(req.params.id);
        if (!deletedProjectWeb) return res.status(404).json({ message: "Projet introuvable" });
